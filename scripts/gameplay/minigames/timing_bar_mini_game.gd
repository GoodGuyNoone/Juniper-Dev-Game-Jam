...
func _randomize_success_zone() -> void:
	var zone_height: float = success_zone.size.y
	var min_y: float = bar.position.y
	var max_y: float = maxf(min_y, bar.position.y + bar.size.y - zone_height)
...
func _move_marker(delta: float) -> void:
	...
	var min_y: float = bar.position.y
	var max_y: float = maxf(min_y, bar.position.y + bar.size.y - marker.size.y)
